...
class QualityGateManager {
     ...
     */
    async evaluateGate(gate, workItem, instance, evaluatedBy) {
        ...
        try {
            ...
            const requiredChecksFailed = checkResults.filter(r => r.isRequired && !r.passed);
     ...
     */
    async evaluateCheck(check, workItem, instance) {
        try {
            const result = await this.executeCheckType(check.type, check.config, workItem, instance);
            return {
                checkId: check.id,
     ...
     */
    async executeCheckType(type, config, workItem, _instance) {
        switch (type) {
            case 'code-coverage':
     ...
     * Check code coverage
     */
    async checkCodeCoverage(config, workItem) {
     ...
     * Check lint errors
     */
    async checkLintErrors(config, workItem) {
     ...
     */
    async checkSecurity(config, workItem) {
        ...
        if (workItem.type === 'feature') {
            const feature = workItem;
            const securityViolations = feature.compliance.violations.filter(v => v.rule.toLowerCase().includes('security'));
     ...
     * Check compliance
     */
    async checkCompliance(config, workItem) {
     ...
     */
    async checkDocumentation(config, workItem) {
        ...
        if (workItem.type === 'feature') {
            hasReadme = workItem.artifacts.documentation?.some((doc) => doc.toLowerCase().includes('readme')) || false;
            hasApiDocs = workItem.artifacts.documentation?.some((doc) => doc.toLowerCase().includes('api') || doc.toLowerCase().includes('docs')) || false;
            hasExamples = workItem.artifacts.documentation?.some((doc) => doc.toLowerCase().includes('example') || doc.toLowerCase().includes('sample')) || false;
        }
     ...
     */
    async checkTestPassRate(config, workItem) {
     ...
     */
    async checkPerformance(config, _workItem) {
     ...
     */
    async checkAccessibility(config, _workItem) {
     ...
     */
    async checkCustom(config, _workItem) {
     ...
     */
    calculateGateScore(gate, checkResults) {
     ...
     */
    async bypassGate(gate, result, bypassedBy, reason, approvedBy) {
        if (!gate.canBypass) {
            return (0, common_1.failure)(new Error('This quality gate cannot be bypassed'));
        }
        if (gate.bypassRequiresApproval && !approvedBy) {
            return (0, common_1.failure)(new Error('Bypass requires approval'));
        }
        const bypassed = {
            ...result,
            bypassed: {
        ...
        return (0, common_1.success)(bypassed);
    }
    /**
     * Get gate status summary
     */
    getGateStatus(result) {
        if (result.bypassed) {
            return {
                status: 'bypassed',
...
//# sourceMappingURL=QualityGateManager.js.map