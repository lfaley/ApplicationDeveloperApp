...
import { z } from 'zod';
import { orchestrateWorkflow, listPatterns, getPatternInfo, listAvailableAgents, validateWorkflow, } from './tools.js';
    ...
    try {
        ...
        switch (name) {
            case 'orchestrate_workflow': {
                const result = await orchestrateWorkflow(args);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'validate_workflow': {
                const validation = validateWorkflow(args);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'run_assessment': {
                ...
                const result = await runAssessment(args);
                return {
                    content: [
                        {
                            type: 'text',
