 ...
 */
export class HandoffPattern {
    type = 'handoff';
    name = 'Handoff Orchestration';
    description = 'dynamic workflow with conditional agent handoffs';
    /**
    ...
    async execute(request) {
        ...
        while (currentIndex < agents.length && iterations < maxIterations) {
            ...
            const nextIndex = this.determineNextAgent(result, agents, currentIndex);
            if (nextIndex === -1) {
    ...
    validate(request) {
        ...
        for (let i = 0; i < request.agents.length; i++) {
            const agent = request.agents[i];
            if (agent.handoffTo) {
                const exists = request.agents.some(a => a.agentId === agent.handoffTo);
                if (!exists) {
                    warnings.push(`Agent ${agent.agentId} has handoffTo target ${agent.handoffTo} which is not in the workflow`);
                }
            }
        }
        return {
     ...
     */
    shouldSkip(agentConfig, previousResults) {
        // Check dependency conditions
        if (agentConfig.dependsOn) {
            const dependency = previousResults.find(r => r.agentId === agentConfig.dependsOn);
            if (!dependency) {
     ...
     */
    determineNextAgent(currentResult, agents, currentIndex) {
    ...
    async executeAgent(agentConfig, previousResults) {
        ...
        try {
            ...
            const output = await this.simulateAgentExecution(agentConfig, previousResults, timeout);
            const endTime = new Date();
            return {
     ...
     */
    async simulateAgentExecution(agentConfig, _previousResults, _timeout) {
     ...
     */
    createSkippedResult(agentConfig) {
        const now = new Date();
        return {
            agentId: agentConfig.agentId,
            status: 'skipped',
     ...
     * @returns Orchestration result
     */
    buildResult(request, agentResults, startTime, endTime) {
