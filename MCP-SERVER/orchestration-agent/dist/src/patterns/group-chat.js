 ...
 */
export class GroupChatPattern {
    type = 'group-chat';
    name = 'Group Chat';
    description = 'Multi-agent discussion with iterative refinement';
    maxRounds = 3; // Default maximum rounds
    minRounds = 1; // Minimum rounds to execute
    /**
     ...
     */
    async executeAgent(agentConfig, previousResults, round) {
        const startTime = new Date();
        ...
        try {
            ...
            const output = await this.simulateAgentExecution(agentConfig, context, timeout);
            const endTime = new Date();
            return {
     ...
     */
    buildDiscussionContext(previousResults, currentRound) {
     ...
     */
    async simulateAgentExecution(agentConfig, context, _timeout) {
