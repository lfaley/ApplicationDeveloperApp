...
export class SequentialPattern {
    ...
    validate(request) {
        ...
        for (let i = 0; i < request.agents.length; i++) {
            const agent = request.agents[i];
            if (agent.dependsOn) {
                const dependencyIndex = request.agents.findIndex(a => a.agentId === agent.dependsOn);
                if (dependencyIndex === -1) {
                    warnings.push(`Agent ${agent.agentId} depends on ${agent.dependsOn} which is not in the workflow`);
                }
                else if (dependencyIndex >= i) {
                    warnings.push(`Agent ${agent.agentId} depends on ${agent.dependsOn} which appears later in the sequence`);
                }
            }
        }
        return {
     ...
     */
    shouldSkip(agentConfig, previousResults, hasFailure, continueOnError) {
     ...
     */
    async executeAgent(agentConfig, receivedContext = false) {
        const startTime = new Date();
     ...
     */
    async simulateAgentExecution(agentConfig, timeout) {
