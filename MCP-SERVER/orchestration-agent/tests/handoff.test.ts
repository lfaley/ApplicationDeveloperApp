...
import { OrchestrationRequest, AgentResult } from '../src/types.js';
import { AgentRegistry } from '../src/registry.js';
import { z } from 'zod';

describe('HandoffPattern', () => {
  let pattern: HandoffPattern;
  ...
  describe('Validation', () => {
    ...
    test('should warn about invalid handoff targets', () => {
      ...
      expect(result.warnings).toContain(
        'Agent agent1 has handoffTo target nonexistent-agent which is not in the workflow'
      );
    });
