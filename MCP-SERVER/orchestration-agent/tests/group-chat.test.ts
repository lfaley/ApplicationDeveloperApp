...
describe('GroupChatPattern', () => {
  let pattern: GroupChatPattern;

  beforeEach(() => {
    pattern = new GroupChatPattern();
    
    // Clear and setup test agents
    (registry as any).agents.clear();
  ...
  describe('Discussion Rounds', () => {
    it('should execute multiple rounds of discussion', async () => {
      ...
      const rounds = result.agentResults.map(r => (r.output as any)?.round);
    ...
    it('should pass discussion context to each agent', async () => {
      ...
      laterResults.forEach(r => {
        if (r.status === 'completed') {
          expect((r.output as any).discussionContext).toBeDefined();
          expect((r.output as any).consideredPreviousOutputs).toBeGreaterThan(0);
        }
      });
    });
    ...
    it('should track round numbers correctly', async () => {
      ...
      const reviewerRounds = reviewerResults.map(r => (r.output as any)?.round);
  ...
  describe('Context Building', () => {
    it('should accumulate previous outputs across rounds', async () => {
      ...
      const laterRoundResult = result.agentResults.find(r => 
        r.status === 'completed' && (r.output as any).round >= 2
      );

      expect(laterRoundResult).toBeDefined();
      expect((laterRoundResult!.output as any).consideredPreviousOutputs).toBeGreaterThan(0);
    });
    ...
    it('should provide summary of discussion history', async () => {
      ...
      const resultsWithContext = result.agentResults.filter(r =>
        r.status === 'completed' && (r.output as any).discussionContext
      );

      expect(resultsWithContext.length).toBeGreaterThan(0);
      resultsWithContext.forEach(r => {
        expect((r.output as any).discussionContext).toContain('Round');
      });
    });
  });
  ...
  describe('Output Aggregation', () => {
    it('should aggregate all results', async () => {
      ...
      expect((result.aggregatedOutput as any).results).toBeDefined();
      expect((result.aggregatedOutput as any).results.length).toBe(result.agentResults.length);
    });
    ...
    it('should provide final round outputs separately', async () => {
      ...
      expect((result.aggregatedOutput as any).finalRoundOutputs).toBeDefined();
      expect((result.aggregatedOutput as any).finalRoundOutputs.length).toBeGreaterThan(0);
      
      // Should have one final output per unique agent
      expect((result.aggregatedOutput as any).finalRoundOutputs.length).toBeLessThanOrEqual(2);
    });
    ...
    it('should filter failed results from final outputs', async () => {
      ...
      const finalOutputs = (result.aggregatedOutput as any).finalRoundOutputs;
      ...
      finalOutputs.forEach((output: any) => {
