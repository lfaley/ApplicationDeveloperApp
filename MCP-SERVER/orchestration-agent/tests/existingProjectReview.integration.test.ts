...
function logDebug(message: any, ...args: any[]): void {
...
describe('Existing Project Review Workflow', () => {
  it('should run the workflow and generate user goals and prompt results files', async () => {
    try {
      ...
      if (fs.existsSync(userGoalsPath)) fs.unlinkSync(userGoalsPath);
      if (fs.existsSync(promptResultsPath)) fs.unlinkSync(promptResultsPath);
