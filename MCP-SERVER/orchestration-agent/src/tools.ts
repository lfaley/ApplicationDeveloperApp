...
import type { OrchestrationPattern, OrchestrationRequest, OrchestrationResult } from './types.js';
...
export async function orchestrateWorkflow(
  ...
  if (!patternValidation.valid) {
    throw new Error(
      `Pattern validation failed: ${patternValidation.errors?.join(', ')}`
    );
  }
 ...
 */
export function getPatternInfo(patternType: string): {
  type: string;
  name: string;
  description: string;
  useCases: string[];
  ...
  const patternInfo = info[patternType as keyof typeof info];
  if (!patternInfo) {
 ...
 */
export function validateWorkflow(request: OrchestrationRequest): {
  valid: boolean;
  errors: string[];
  warnings: string[];
} {
