...
import { VelocityCalculator, ForecastCalculator } from '../calculators';
import { RoadmapAnalysis, Blocker, Risk } from '../types';

export async function analyzeRoadmap(roadmapContent: string): Promise<RoadmapAnalysis> {
 ...
 */
function identifyBlockers(features: import('../types').RoadmapFeature[]): Blocker[] {
 ...
 */
function categorizeBlocker(blocker: string): Blocker['blockerType'] {
  const lower = blocker.toLowerCase();
  
  if (lower.includes('depend')) return 'dependency';
  if (lower.includes('resource') || lower.includes('team')) return 'resource';
  if (lower.includes('external') || lower.includes('vendor')) return 'external';
  return 'technical';
}
...
function assessRisks(
  features: import('../types').RoadmapFeature[],
  velocity: import('../types').VelocityMetrics,
