...
exports.ForecastCalculator = void 0;
class ForecastCalculator {
    /**
     * Generate project completion forecast
     */
    generateForecast(features, velocity, remainingWork) {
     ...
     */
    estimateDate(sprintsRemaining, sprintLength = 14) {
     ...
     */
    assessConfidence(velocity, features) {
        let score = 0;
        ...
        if (velocity.trend === 'stable')
            ...
            score += 1;
        // Forecast accuracy
        if (velocity.forecastAccuracy > 85)
            score += 2;
        else if (velocity.forecastAccuracy > 70)
            score += 1;
        // Sprint history
        if (velocity.lastSprints.length >= 5)
            score += 1;
        else if (velocity.lastSprints.length >= 3)
            score += 0.5;
        // Blocked features
     ...
     */
    monteCarloForecast(remainingWork, velocityHistory, simulations = 1000) {
     ...
     */
    sampleVelocity(velocityHistory) {
