...
import type { ContextHealthScore, DriftDetectionResult } from './types.js';
...
export class HealthChecker {
   ...
   */
  private calculateCoverage(driftResult: DriftDetectionResult): number {
   ...
   */
  private calculateFreshness(driftResult: DriftDetectionResult): number {
   ...
   */
  private calculateConsistency(driftResult: DriftDetectionResult): number {
    const inconsistentCount = driftResult.driftItems.filter(
      d => d.type === 'orphaned' || d.type === 'inconsistent'
    ).length;
   ...
   */
  private calculateCompleteness(driftResult: DriftDetectionResult): number {
    // Based on severity distribution
    const { critical, high, medium, low, totalDrifts } = driftResult.summary;

    if (totalDrifts === 0) return 100;
   ...
   */
  private identifyStrengths(factors: {
    documentationCoverage: number;
    documentationFreshness: number;
    consistencyScore: number;
    completenessScore: number;
