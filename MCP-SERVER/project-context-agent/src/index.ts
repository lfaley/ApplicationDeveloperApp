...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'detect_drift': {
        const { workspacePath, includePatterns, excludePatterns, severityThreshold } = args as {
      ...
      case 'health_check': {
        const { workspacePath, includePatterns } = args as {
          workspacePath: string;
          includePatterns?: string[];
        };
      ...
      case 'auto_sync': {
        const { workspacePath, driftItemIds, requireApproval, createBackup } = args as {
          workspacePath: string;
          driftItemIds?: string[];
