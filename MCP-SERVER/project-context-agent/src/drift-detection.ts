...
export class DriftDetector {
  ...
  async detectDrift(options: {
    ...
    const changedCodeFiles = await this.git.getChangedFilesSince(
      thirtyDaysAgo,
      CODE_PATTERNS
    );
    ...
    const changedDocFiles = await this.git.getChangedFilesSince(
      thirtyDaysAgo,
      DOC_PATTERNS
    );
   ...
   */
  private async checkFilePairForDrift(
    codeFile: string,
    docFile: string
   ...
   */
  private async createMissingDocDrift(codeFile: string): Promise<DriftItem> {
    const codeInfo = await this.git.getFileModificationInfo(codeFile);
   ...
   */
  private async createOrphanedDocDrift(docFile: string): Promise<DriftItem | null> {
    const docInfo = await this.git.getFileModificationInfo(docFile);
   ...
   */
  private async findRelatedDocumentation(codeFile: string): Promise<string[]> {
   ...
   */
  private async findRelatedCode(docFile: string): Promise<string[]> {
   ...
   */
  private calculateSeverity(daysDiff: number, linesChanged: number): DriftSeverity {
   ...
   */
  private generateRecommendations(driftItems: DriftItem[]): string[] {
 ...
 */
export async function createDriftDetector(git: GitIntegration): Promise<DriftDetector> {
