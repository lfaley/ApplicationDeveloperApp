 ...
 */
function validatePath(repoPath: string): boolean {
 ...
 */
async function handleDetectDrift(repoPath: string): Promise<void> {
  ...
  try {
    ...
    if (result.hasDrift) {
      ...
      result.driftItems.forEach(item => {
        const severityColor = 
          item.severity === 'critical' ? colors.red :
 ...
 */
async function handleHealthCheck(repoPath: string): Promise<void> {
 ...
 */
async function handleSuggestUpdates(repoPath: string): Promise<void> {
 ...
 */
async function handleAutoSync(repoPath: string, args: string[]): Promise<void> {
...
main().catch(error => {
  print(colors.red, `❌ Fatal error: ${(error as Error).message}`);
  process.exit(1);
});
