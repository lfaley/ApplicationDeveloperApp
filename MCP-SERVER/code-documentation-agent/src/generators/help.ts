...
import type { FunctionInfo, GenerateOptions } from '../types.js';

/**
 * Generate PowerShell help comment for a function
 */
export function generateHelpForFunction(
 ...
 */
function inferSynopsis(funcInfo: FunctionInfo): string {
 ...
 */
function inferDescription(funcInfo: FunctionInfo): string {
 ...
 */
function inferOutputDescription(funcInfo: FunctionInfo): string {
  const name = funcInfo.name;
 ...
 */
function generateExamples(funcInfo: FunctionInfo): Array<{ code: string; description?: string }> {
  ...
  const basicArgs = funcInfo.params
    .filter(p => !p.optional)
    .map(p => {
      if (p.type?.toLowerCase().includes('string')) return '-' + p.name + ' "value"';
      if (p.type?.toLowerCase().includes('int')) return '-' + p.name + ' 42';
      if (p.type?.toLowerCase().includes('switch') || p.type?.toLowerCase().includes('bool')) return '-' + p.name;
      return '-' + p.name + ' $value';
    })
    .join(' ');
