 ...
 */
export function generateXmlDocForFunction(
 ...
 */
export function generateXmlDocForClass(
  classInfo: ClassInfo,
  options: GenerateOptions
): string {
  const lines: string[] = [];
 ...
 */
export function generateXmlDocForInterface(
 ...
 */
export function generateXmlDocForProperty(
...
function generateFunctionExample(funcInfo: FunctionInfo): string {
  const args = funcInfo.params.map(p => {
    if (p.type?.toLowerCase().includes('string')) return '"example"';
    if (p.type?.toLowerCase().includes('int') || p.type?.toLowerCase().includes('long')) return '42';
    if (p.type?.toLowerCase().includes('double') || p.type?.toLowerCase().includes('decimal')) return '3.14';
    if (p.type?.toLowerCase().includes('bool')) return 'true';
    if (p.type?.toLowerCase().includes('list') || p.type?.toLowerCase().includes('array')) return 'new[]{}';
    if (p.type?.toLowerCase().includes('dictionary')) return 'new Dictionary<,>()';
    return 'value';
  }).join(', ');
 ...
 */
function inferInterfaceDescription(interfaceInfo: InterfaceInfo): string {
 ...
 */
function inferPropertyValueDescription(propName: string): string {
