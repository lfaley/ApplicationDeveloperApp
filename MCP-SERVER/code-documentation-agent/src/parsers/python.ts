...
import type { FunctionInfo, ClassInfo, Parameter, AnalysisResult } from '../types.js';

/**
 * Parse Python code and extract documentation information
 */
export async function parsePython(code: string, filePath: string = '<input>'): Promise<AnalysisResult> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (classMatch) {
      ...
      for (let j = i + 1; j < endLine; j++) {
        ...
        if (methodMatch) {
          ...
          for (let k = j + 1; k < endLine; k++) {
            ...
            if (nextMethodLine.length === 0) continue;
      ...
      const properties: Array<{ name: string; type: string | null; isReadonly: boolean; isStatic: boolean; visibility: 'public' | 'private' | 'protected'; hasDocumentation: boolean }> = rawProperties.map(p => ({
        name: p.name,
        type: p.type || null,
        isReadonly: false,
        isStatic: p.isStatic,
        visibility: p.visibility as 'public' | 'private' | 'protected',
 ...
 */
function extractDecorators(lines: string[], defLine: number): string[] {
 ...
 */
function extractThrows(lines: string[], startLine: number, endLine: number): string[] {
...
function calculateComplexity(lines: string[], startLine: number, endLine: number): number {
  ...
  for (let i = startLine; i < endLine; i++) {
    ...
    if (line.startsWith('if ') || line.startsWith('elif ')) complexity++;
    if (line.startsWith('for ') || line.startsWith('while ')) complexity++;
    if (line.startsWith('except ')) complexity++;
    if (line.includes(' and ') || line.includes(' or ')) complexity++;
    if (line.includes('if ') && line.includes(' else ')) complexity++; // Ternary
  }
 ...
 */
function extractClassAttributes(lines: string[], startLine: number, endLine: number): Array<{ name: string; type?: string; isStatic: boolean; visibility: string }> {
  const attributes: Array<{ name: string; type?: string; isStatic: boolean; visibility: string }> = [];
 ...
 */
function calculateDocumentedLines(functions: FunctionInfo[], classes: ClassInfo[]): number {
  let documented = 0;
