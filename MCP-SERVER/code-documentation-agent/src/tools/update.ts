 ...
 * Tool handler
 */
export async function updateCommentsTool(args: unknown) {
  try {
...
function findTarget(
  ...
  targetName: string,
  language: string
): { line: number; type: 'function' | 'class' } | null {
 ...
 */
function findExistingDocumentation(
  lines: string[],
  targetLine: number,
  language: string
...
function generateDiff(
  ...
  const diff: string[] = ['--- Original', '+++ Modified', `@@ -${start + 1} +${start + 1} @@`];
