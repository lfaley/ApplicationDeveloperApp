 ...
 */
export function generateDocstringForFunction(funcInfo, options, style = 'google') {
    if (style === 'google') {
 ...
 * """
 */
function generateGoogleDocstring(funcInfo, options) {
    const lines = ['"""'];
    // Description
 ...
 * """
 */
function generateNumpyDocstring(funcInfo, options) {
    const lines = ['"""'];
    // Description
    if (options.includeDescription) {
        const description = options.inferDescriptions
 ...
 * """
 */
function generateSphinxDocstring(funcInfo, options) {
    const lines = ['"""'];
    // Description
    if (options.includeDescription) {
        const description = options.inferDescriptions
...
function generateFunctionExample(funcInfo) {
    const args = funcInfo.params
        ...
        if (p.type === 'bool' || p.type === 'boolean')
            return 'True';
        if (p.type === 'list' || p.type?.includes('List'))
            return '[]';
        if (p.type === 'dict' || p.type?.includes('Dict'))
            return '{}';
        return 'value';
    })
        .join(', ');
 ...
 */
export function generateDocstringForClass(classInfo, options, style = 'google') {
    const lines = ['"""'];
    // Description
    if (options.includeDescription) {
        const description = options.inferDescriptions
