 ...
 */
export function generateJSDocForFunction(func, options) {
    const lines = ['/**'];
    // Description
 ...
 */
export function generateJSDocForClass(cls, options) {
    const lines = ['/**'];
    // Description
    if (options.includeDescription) {
 ...
 */
export function generateJSDocForInterface(iface, options) {
    const lines = ['/**'];
    // Description
    if (options.includeDescription) {
 ...
 */
function inferFunctionDescription(func) {
 ...
 */
function inferParameterDescription(paramName, funcName) {
    ...
    if (name === 'key')
        return 'The key';
    if (name === 'file')
        return 'The file path or object';
    if (name === 'path')
    ...
    // Pattern-based
    if (name.endsWith('id'))
        return `The ${name.slice(0, -2)} identifier`;
    if (name.endsWith('name'))
        return `The ${name.slice(0, -4)} name`;
    if (name.endsWith('path'))
        return `The ${name.slice(0, -4)} path`;
    if (name.endsWith('count'))
        return `The number of ${name.slice(0, -5)}`;
    if (name.endsWith('list') || name.endsWith('array'))
        return `Array of ${name.slice(0, -4)}`;
 ...
 */
function inferReturnDescription(func) {
    const name = func.name.toLowerCase();
    const returnType = func.returnType || 'any';
    if (name.startsWith('get'))
        return `The retrieved ${name.slice(3) || 'data'}`;
    if (name.startsWith('is') || name.startsWith('has') || name.startsWith('can')) {
        return `True if condition is met, false otherwise`;
    }
    if (name.startsWith('create'))
        return `The newly created ${name.slice(6) || 'object'}`;
    if (name.startsWith('calculate') || name.startsWith('compute')) {
        return `The calculated ${name.slice(9) || 'result'}`;
    }
    if (name.startsWith('find'))
        return `The found ${name.slice(4) || 'item'} or null`;
    if (name.startsWith('filter'))
        return `Filtered array of items`;
    if (name.startsWith('map'))
        return `Mapped array of items`;
    if (returnType === 'boolean')
        return 'True if successful, false otherwise';
    if (returnType === 'Promise')
        return 'Promise that resolves with the result';
    if (returnType.endsWith('[]'))
        return `Array of ${returnType.slice(0, -2)}`;
    return `The ${returnType} result`;
}
/**
 * Infer throw condition
 */
function inferThrowCondition(exceptionType, func) {
    ...
    if (exceptionType === 'UnauthorizedError')
    ...
    if (exceptionType === 'TypeError')
        return 'invalid type is provided';
    if (exceptionType === 'RangeError')
 ...
 */
function inferClassDescription(cls) {
    const name = cls.name;
    const words = name.replace(/([A-Z])/g, ' $1').trim();
    if (name.endsWith('Service'))
        return `Service for managing ${words.slice(0, -7).toLowerCase()}`;
    if (name.endsWith('Controller'))
        return `Controller for handling ${words.slice(0, -10).toLowerCase()} requests`;
    if (name.endsWith('Repository'))
        return `Repository for ${words.slice(0, -10).toLowerCase()} data access`;
    if (name.endsWith('Manager'))
        return `Manager for ${words.slice(0, -7).toLowerCase()} operations`;
    if (name.endsWith('Helper'))
        return `Helper class for ${words.slice(0, -6).toLowerCase()} utilities`;
    if (name.endsWith('Validator'))
        return `Validator for ${words.slice(0, -9).toLowerCase()}`;
    if (name.endsWith('Builder'))
        return `Builder for constructing ${words.slice(0, -7).toLowerCase()}`;
    if (name.endsWith('Factory'))
        return `Factory for creating ${words.slice(0, -7).toLowerCase()} instances`;
    return `${name} class`;
}
/**
 * Infer interface description
 */
function inferInterfaceDescription(iface) {
    const name = iface.name;
    const words = name.replace(/([A-Z])/g, ' $1').trim();
    if (name.endsWith('Config'))
        return `Configuration for ${words.slice(0, -6).toLowerCase()}`;
    if (name.endsWith('Options'))
        return `Options for ${words.slice(0, -7).toLowerCase()}`;
    if (name.endsWith('Props'))
        return `Props for ${words.slice(0, -5).toLowerCase()} component`;
    if (name.endsWith('State'))
        return `State for ${words.slice(0, -5).toLowerCase()}`;
    if (name.endsWith('Response'))
        return `Response from ${words.slice(0, -8).toLowerCase()}`;
    if (name.endsWith('Request'))
        return `Request for ${words.slice(0, -7).toLowerCase()}`;
 ...
 */
function generateFunctionExample(func) {
 ...
 */
function generateClassExample(cls) {
