 ...
 * Tool handler
 */
export async function validateDocumentationTool(args) {
    try {
        ...
        const result = await validateDocumentation(code, validated.language, validated.filePath || '<input>', validated.rules);
        // Format output
        const output = [];
        ...
        if (result.missingDocs.length > 0) {
            ...
            result.missingDocs.forEach((item) => {
                output.push(`  • ${item}`);
            });
        }
        return {
 ...
 */
async function validateDocumentation(code, language, filePath, rules) {
    const defaultRules = {
        requireParams: true,
        requireReturns: true,
 ...
 */
function validateFunction(func, rules, errors, warnings, info) {
 ...
 */
function validateClass(cls, rules, errors, warnings, info) {
    ...
    const publicMethods = cls.methods.filter((m) => m.isExported);
    const undocumentedPublicMethods = publicMethods.filter((m) => !m.hasDocumentation);
