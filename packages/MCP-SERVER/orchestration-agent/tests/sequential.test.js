...
describe('SequentialPattern', () => {
    ...
    describe('Validation', () => {
        ...
        test('should warn about invalid dependency references', () => {
            ...
            const result = pattern.validate(request);
            expect(result.warnings).toContain('Agent code-review depends on nonexistent-agent which is not in the workflow');
        });
        test('should warn when dependent agent appears before dependency', () => {
            ...
            const result = pattern.validate(request);
            expect(result.warnings).toContain('Agent code-review depends on code-documentation which appears later in the sequence');
        });
